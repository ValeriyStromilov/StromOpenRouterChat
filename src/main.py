...
from ui.components import *                         # Компоненты пользовательского интерфейса
from utils.cache import ChatCache, CacheManager     # Модуль для кэширования истории чата
...
async def main(page: ft.Page):
    page.title = "AI Chat Application"
    page.window.width = 800
    page.window.height = 600

    # Определение состояния: есть ли зарегистрированный API ключ
    cache_manager = CacheManager()
    has_api_key = cache_manager.has_auth_data()

    # Переключение на нужный компонент
    if not has_api_key:
        # Пользователь впервые открывает приложение, начинаем с регистрации
        reg_component = RegistrationComponent(page, register_callback=lambda pin_code: register_and_open_main(page, pin_code))
        page.add(reg_component)
    else:
        # Пользователь уже зарегистрирован, предлагаем войти по PIN-коду
        login_component = LoginComponent(page, login_callback=lambda: login_and_open_main(page))
        page.add(login_component)

    page.update()

def login_and_open_main(page):
    """
    Callback после успешного входа по PIN-коду.
    """
